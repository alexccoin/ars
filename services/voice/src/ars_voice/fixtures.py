...
@dataclass
class SpokenSpec:
    """A fixture made of real synthesised speech rather than speech-shaped noise.

    Generated by running the configured TTS engine — Piper, real EN and RO voices — and
    splicing the result onto a room-noise floor. Still not a human in a kitchen, but it has
    real phonetics, real prosody and real coarticulation, which is what wakeword models and
    whisper actually respond to. Speech-shaped noise cannot exercise either.

    The wake phrase is English in both fixtures on purpose: the keyword is English
    (`hey_jarvis`) and a Romanian speaker still says it in English before switching.
    """
    ...
    command: str
    lead_in_ms: int = 500
    trailing_ms: int = 1_500
    noise_floor_dbfs: float = -62.0
    description: str = ""


def spoken_specs() -> list[SpokenSpec]:
    return [
        SpokenSpec(
            name="en_bank_messages",
            language=Language.EN,
            wake_phrase="Hey Jarvis.",
            command="Good morning. I found three new messages from the bank. "
                    "Would you like me to read them?",
            description="English wake + English command.",
        ),
        SpokenSpec(
            name="ro_bank_messages",
            language=Language.RO,
            wake_phrase="Hey Jarvis.",
            command="Bună dimineața! Am găsit 3 mesaje noi de la bancă. Vrei să ți le citesc?",
            description="English wake + Romanian command — the code-switch case.",
        ),
        SpokenSpec(
            name="en_short_command",
            language=Language.EN,
            wake_phrase="Hey Jarvis.",
            command="Turn the lights off in the kitchen.",
            description="Short English command.",
        ),
        SpokenSpec(
            name="ro_short_command",
            language=Language.RO,
            wake_phrase="Hey Jarvis.",
            command="Stinge lumina din bucătărie.",
            description="Short Romanian command.",
        ),
    ]


async def render_spoken(tts, spec: SpokenSpec) -> tuple[bytes, dict]:
    """Render one spoken fixture with a `TtsEngine`.

    Takes the engine rather than importing Piper: `packages/core`'s rule is that a vendor SDK
    only ever lives behind an interface, and that applies to test data too.
    """
    from ars_protocol import Language as _Language
    from ars_protocol import SynthesisRequest

    async def say(text: str, language) -> bytes:
        pcm = b""
        async for chunk in tts.synthesize(SynthesisRequest(text=text, language=language)):
            pcm += chunk.pcm
        return pcm

    wake = await say(spec.wake_phrase, _Language.EN)
    command = await say(spec.command, spec.language)
    lead = room_noise_pcm(spec.lead_in_ms, level_dbfs=spec.noise_floor_dbfs, seed=1)
    tail = room_noise_pcm(spec.trailing_ms, level_dbfs=spec.noise_floor_dbfs, seed=2)
    pcm = lead + wake + command + tail
    labels = {
        "wake_start_ms": spec.lead_in_ms,
        "wake_end_ms": spec.lead_in_ms + int(len(wake) / 32),
        "command_start_ms": spec.lead_in_ms + int(len(wake) / 32),
        "speech_end_ms": spec.lead_in_ms + int((len(wake) + len(command)) / 32),
        "total_ms": int(len(pcm) / 32),
        "expected_text": spec.command,
        "expected_language": spec.language.value,
    }
    return pcm, labels


async def generate_spoken(tts, root: Path | str = DEFAULT_DIR, *, force: bool = False) -> Path:
    directory = Path(root) / "spoken"
    directory.mkdir(parents=True, exist_ok=True)
    manifest = Manifest(kind="spoken", synthetic=True, generator="ars_voice.fixtures + TTS")
    for spec in spoken_specs():
        path = directory / f"{spec.name}.wav"
        entry = asdict(spec)
        entry["language"] = spec.language.value
        entry["file"] = path.name
        if force or not path.is_file():
            pcm, labels = await render_spoken(tts, spec)
            write_wav(path, pcm)
            entry.update(labels)
            (directory / f"{spec.name}.json").write_text(
                json.dumps(labels, indent=2, ensure_ascii=False) + "\n"
            )
        elif (directory / f"{spec.name}.json").is_file():
            entry.update(json.loads((directory / f"{spec.name}.json").read_text()))
...
def load_spoken(root: Path | str = DEFAULT_DIR) -> list[tuple[dict, Path]]:
    directory = Path(root) / "spoken"
    manifest_path = directory / "manifest.json"
    if not manifest_path.is_file():
        raise FileNotFoundError(
            f"no spoken fixtures at {manifest_path} — run `ars-voice-fixtures spoken` "
            "(needs Piper voices in models/tts)"
        )
    manifest = json.loads(manifest_path.read_text())
    return [(entry, directory / entry["file"]) for entry in manifest["fixtures"]]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "kind",
        choices=["endpointing", "wakeword_negative", "wakeword_positive", "spoken", "all"],
    )
    ...
    kinds = (
        ["endpointing", "wakeword_negative", "wakeword_positive", "spoken"]
        if args.kind == "all"
        else [args.kind]
    )
    for kind in kinds:
        if kind == "spoken":
            ...
            if config.tts.backend != "piper":
                print("skipping 'spoken': needs ARS_TTS_BACKEND=piper and models/tts")
                continue
            path = asyncio.run(
                generate_spoken(build_tts(config), args.root, force=args.force)
            )
        else:
